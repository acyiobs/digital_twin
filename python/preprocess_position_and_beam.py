...
print(f"Number real data points: {dataframe.shape[0]}")
...
savemat("data/ue_relative_pos.mat", {"ue_relative_pos": pos_diff})
savemat("data/real_beam_pwr.mat", {"real_beam_pwr": pwrs_array})
savemat("data/ue_gps_pos.mat", {"ue_gps_pos": pos_ue_array})

print("UE position and beam power are preproccesed and saved")
...
rect1 = Rectangle(
    (x_min, y_min), height, width, linewidth=1, edgecolor="r", facecolor="none"
)
...
k = np.sum(center_ue_pos[:, 1] * center_ue_pos[:, 0]) / np.sum(
    center_ue_pos[:, 0] * center_ue_pos[:, 0]
)
