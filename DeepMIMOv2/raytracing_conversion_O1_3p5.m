...
for ii=1:1:num_BS % For each BS 
    ...
    sfile_DoD=strcat('Raytracing_scenarios/',scenario_write,'/',scenario_write,'.',num2str(ii),'.DoD.mat');
    sfile_DoA=strcat('Raytracing_scenarios/',scenario_write,'/',scenario_write,'.',num2str(ii),'.DoA.mat');
    sfile_CIR=strcat('Raytracing_scenarios/',scenario_write,'/',scenario_write,'.',num2str(ii),'.CIR.mat'); 
...
for ii=1:1:num_TX_BS % For each BS transmitter
    ...
    % Name of the output DoD & CIR files
    sfile_DoD=strcat('Raytracing_scenarios/',scenario_write,'/',scenario_write,'.',num2str(ii),'.DoD.BSBS.mat');
    sfile_DoA=strcat('Raytracing_scenarios/',scenario_write,'/',scenario_write,'.',num2str(ii),'.DoA.BSBS.mat');
    sfile_CIR=strcat('Raytracing_scenarios/',scenario_write,'/',scenario_write,'.',num2str(ii),'.CIR.BSBS.mat'); 
    % Concatenate the number of points to the final output arrays
...
Loc_array_full=[];
disp('Reading and writing the Loc files for the users ...')
...
RX_Loc_array_full=[];
disp('Reading and writing the Loc files for all the receivers ...')
% Read one Loc file from one BS
for jj=1:1:num_user_grids % For each user grid
   filename_Loc=strcat('Raytracing_scenarios/',scenario_read,'/',scenario_read,'.pl.t001_',TX_ID_BS_str{1},'.r',RX_grids_ID_user_str{jj},'.p2m');
   Loc_array=importdata(filename_Loc);
...
TX_Loc_array_full=[];
disp('Reading and writing the Loc files for all the transmitters ...')
% Read one Loc file from one BS
