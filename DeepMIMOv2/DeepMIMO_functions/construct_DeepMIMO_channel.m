...
% Limit the AOAs and AODs from the panel 
Cond_1 =  wrapTo360(params_user.DoD_phi) > 0 & wrapTo360(params_user.DoD_phi) < 180 ;
Cond_2 =  wrapTo360(params_user.DoA_phi) > 180 & wrapTo360(params_user.DoA_phi) < 360 ;
%Cond_2 =  wrapTo360(params_user.DoA_phi) > 0 & wrapTo360(params_user.DoA_phi) < 180 ;
% power(Cond_1 | Cond_2) = 0;

Cond = ones(size(Cond_1));
[~, tt] = min(Cond_1 | Cond_2);
Cond(tt) = 0;
power(logical(Cond)) = 0;
